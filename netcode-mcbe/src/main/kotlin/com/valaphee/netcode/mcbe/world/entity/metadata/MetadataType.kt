...
interface MetadataType<T> {
    ...
    companion object {
        ...
        val Nbt = object : MetadataType<Any?> {
            override fun read(buffer: PacketBuffer) = buffer.nbtObjectMapper.readValue<Any?>(ByteBufInputStream(buffer.buffer) as InputStream)

            override fun write(buffer: PacketBuffer, value: Any?) {
                buffer.nbtObjectMapper.writeValue(ByteBufOutputStream(buffer.buffer) as OutputStream, value)
            }
        }
