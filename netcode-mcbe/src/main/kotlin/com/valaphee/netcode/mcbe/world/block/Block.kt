    ...
    object Serializer : JsonSerializer<Block>() {
        ...
        private fun serializeComponent(value: Map<String, Any?>, version: Int) = value.entries.mapNotNull { (componentKey, component) ->
            when (componentKey) {
                ...
                "minecraft:aim_collision", "minecraft:pick_collision"                                                              -> (                                                         if (version >= V1_18_010) "minecraft:aim_collision"   else "minecraft:pick_collision"  ) to mapOf("enabled" to true, "origin" to (component as Map<*, *>)["origin"].let { listOf(((it as List<*>)[0] as Number).toFloat(), (it[1] as Number).toFloat(), (it[2] as Number).toFloat()) }, "size" to component["size"].let { listOf(((it as List<*>)[0] as Number).toFloat(), (it[1] as Number).toFloat(), (it[2] as Number).toFloat()) })
