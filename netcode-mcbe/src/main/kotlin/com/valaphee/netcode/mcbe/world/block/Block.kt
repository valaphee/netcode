...
class Block : Data {
    ...
    object Serializer : JsonSerializer<Block>() {
        ...
        private fun serializeComponent(value: Map<String, Any?>, version: Int) = value.entries.mapNotNull { (componentKey, component) ->
            when (componentKey) {
                "minecraft:block_light_filter", "minecraft:block_light_absorption" -> if (version >= 486) "minecraft:block_light_filter" to mapOf("lightLevel" to (component as Number).toByte()) else "minecraft:block_light_absorption" to mapOf("value" to (component as Number).toFloat())
                ...
                "minecraft:aim_collision", "minecraft:pick_collision" -> (if (version >= 486) "minecraft:aim_collision" else "minecraft:pick_collision") to mapOf("enabled" to true, "origin" to (component as Map<*, *>)["origin"].let { listOf(((it as List<*>)[0] as Number).toFloat(), (it[1] as Number).toFloat(), (it[2] as Number).toFloat()) }, "size" to component["size"].let { listOf(((it as List<*>)[0] as Number).toFloat(), (it[1] as Number).toFloat(), (it[2] as Number).toFloat()) })
