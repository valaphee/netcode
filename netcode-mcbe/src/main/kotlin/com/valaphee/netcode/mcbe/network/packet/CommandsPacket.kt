...
object CommandsPacketReader : PacketReader {
    override fun read(buffer: PacketBuffer, version: Int): CommandsPacket {
        ...
        val commandStructures = safeList(buffer.readVarUInt()) {
            ...
            Command.Builder(name, description, flags, permission, aliasesIndex, overloadStructures)
        }
        ...
        return CommandsPacket(commandStructures.map {
            ...
            Command(it.name, it.description, it.flags, it.permission, if (aliasesIndex == -1) null else enumerations[aliasesIndex], it.overloads.map {
                it.map {
