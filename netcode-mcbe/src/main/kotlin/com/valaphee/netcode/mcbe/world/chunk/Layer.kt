...
class Layer(
    ...
    fun writeToBuffer(buffer: PacketBuffer, runtime: Boolean) {
        ...
        if (runtime) palette.forEach { buffer.writeVarInt(it) }
            ...
            palette.forEach { buffer.nbtObjectMapper.writeValue(stream, buffer.registrySet.blockStates[it]) }
        }
    }
}
...
fun PacketBuffer.readLayer(): Layer {
    ...
    return Layer(IntArrayList().apply {
        if (runtime) repeat(paletteSize) { add(readVarInt()) }
            ...
            val stream = ByteBufInputStream(buffer) as InputStream
            repeat(paletteSize) { add(registrySet.blockStates.getId(nbtObjectMapper.readValue(stream))) }
        }
    }, blocks)
}
