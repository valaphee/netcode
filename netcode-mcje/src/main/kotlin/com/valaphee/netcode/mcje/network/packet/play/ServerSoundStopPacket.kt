...
class ServerSoundStopPacket(
    ...
    object Reader : Packet.Reader {
        override fun read(buffer: PacketBuffer, version: Int): ServerSoundStopPacket {
            ...
            val category = if (flagsValue and flagHasCategory != 0) SoundCategory.values()[buffer.readVarInt()] else null
            val soundKey = if (flagsValue and flagHasSound != 0) buffer.readNamespacedKey() else null
            return ServerSoundStopPacket(category, soundKey)
        }
    }

    companion object {
        private const val flagHasCategory = 1 shl 0
        private const val flagHasSound    = 1 shl 1
    }
}
